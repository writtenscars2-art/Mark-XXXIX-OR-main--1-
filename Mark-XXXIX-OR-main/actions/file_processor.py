...
def _claude_client():
    ...
    # Return a simple namespace that mimics the old .generate_content() interface
    class _Compat:
        def generate_content(self, prompt_or_list):
            if isinstance(prompt_or_list, list):
                # [text_prompt, PIL.Image] or [text_prompt, {mime_type, data}]
                text_parts = []
                img_bytes = None
                mime = "image/png"
                for p in prompt_or_list:
                    if isinstance(p, str):
                        text_parts.append(p)
                    elif hasattr(p, "tobytes"):          # PIL Image
                        import io
                        buf = io.BytesIO()
                ...
                prompt = "\n".join(text_parts)
                resp_text = _gen(prompt, image_bytes=img_bytes, mime_type=mime)
            else:
                resp_text = _gen(str(prompt_or_list))
...
def _process_text_doc(path: Path, file_type: str, action: str,
    ...
    if action not in prompt_map:
        ...
        action      = "custom"
        instruction = params.get("instruction", "") or original_action
    ...
    if action == "extract":
        ...
        try:
            ...
            if ext == ".zip":
                ...
                return f"Unsupported archive format: {ext}"
            return f"Extracted {count} files to: {dest}"
        except Exception as e:
